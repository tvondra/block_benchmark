<?php
	...
	if (in_array($img, array('tpch-m-time','tpch-time','tpch-score','tpch-m-score','tpch-m-time-pct','tpch-time-pct','tpch-score-pct','tpch-m-score-pct'))) {
		...
		if (is_null($queries) && is_null($mins)) {
			...
			if (in_array($img, array('tpch-m-time','tpch-m-score','tpch-m-time-pct','tpch-m-score-pct'))) {
				$r = get_common_queries(array('config_id = ' . (int)$config . " AND fs = '" . pg_escape_string($fs) . "'"));
			} else {
				$r = get_all_queries(array('config_id = ' . (int)$config . " AND fs = '" . pg_escape_string($fs) . "'"));
			}
		...
		$sql .= ' FROM ' . $columns[$img]['table'] . ' x JOIN config ON (x.config_id = config.id) WHERE config_id = ' . (int)$config . " AND fs = '" . pg_escape_string($fs) . "'";
	} else {
		$sql = 'SELECT title, fs, fs_block, db_block, ' . $columns[$img]['column'] . ' AS val FROM ' . $columns[$img]['table'] . ' x JOIN config ON (x.config_id = config.id) WHERE config_id = ' . (int)$config . " AND fs = '" . pg_escape_string($fs) . "'";
	}
	...
	header('Content-Disposition: inline; filename="' . $fs . '-' . $img . '.png"');
