...
if [ "2051752" != "$mem" ]; then
	echo "ERROR : invalid amount of memory (have you set the 'mem' option?)"
	exit;
fi
...
# pgbench benchmark 
PGB_CONFIG=./postgresql-ssd-pgbench.conf
PGB_CLIENTS=20
...
DSS_CONFIG=./postgresql-ssd-tpch.conf
...
do
	...
	do
		...
		sudo /sbin/mkfs -t ext2 -b $fsbs -E stripe-width=$(((512*1024)/$fsbs)) $DEVICE > mkfs.log 2> mkfs.err
	...
	do
		...
		do
			...
			do
				...
				print_log "creating filesystem"
				sudo /sbin/mkfs -t ext3 -b $fsbs -E stripe-width=$(((512*1024)/$fsbs)) $DEVICE > mkfs.log 2> mkfs.err
	...
	do
		...
		do
			...
			do
				...
				print_log "creating filesystem"
				sudo /sbin/mkfs -t ext4 -b $fsbs -E stripe-width=$(((512*1024)/$fsbs)) $DEVICE > mkfs.log 2> mkfs.err

				fs_prepare noatime,data=$data,$btype,discard
				benchmark_run $pgbs $logdir
				fs_clean $logdir
...
do
	...
	do
		...
		do
			...
			fs_prepare noatime,discard,$btype
			benchmark_run $pgbs $logdir
			fs_clean $logdir
